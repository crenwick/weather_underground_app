...
module.exports = function() {
  return function(req, res, next) {
    ...
    console.log(url);
    var urlSeattle = 'http://api.wunderground.com/api/' + apikey + '/conditions/q/WA/Seattle.json';
    ...
    request.get(urlSeattle)
    .end(function(err, data) {
      if (err) console.log(err);
      var parsedData = JSON.parse(data.text);
      req.locSeattle = parsedData.current_observation.display_location.full;
      req.tempSeattle = parsedData.current_observation.temp_f;
      req.precipSeattle = parseFloat(parsedData.current_observation.precip_today_in);
      ready();
    });
  };
};
